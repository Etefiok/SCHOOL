...
const VideoPlayer2 = ({ content, updatedAt, video2, video, title, topic  }) => {
  const [commentsEcons, setCommentsEcons] = useState([]);
  ...
  const [totalCommentsEcons, settotalCommentsEcons] = useState(0);
    ...
    if (replyText.trim() !== "") {
      try {
        const response = await axios.post(
          `http://localhost:5000/auth/commentsEcons/${commentId}/replies`,
          {
            content: replyText,
            Username: user.Username,
          }
        );
        setCommentsEcons((prevComments) =>
  ...
  const fetchComments = async () => {
    try {
      const response = await axios.get("http://localhost:5000/auth/commentsEcons");
      setCommentsEcons(response.data);
    } catch (error) {
  ...
  const fetchTotalComments = async () => {
    try {
      const response = await axios.get(
        "http://localhost:5000/auth/commentsEcons/count"
      );
      settotalCommentsEcons(response.data.totalCommentsEcons);
    } catch (error) {
  ...
  const handleCommentSubmit = async (event) => {
    ...
    if (newComment.trim() !== "") {
      try {
        const response = await axios.post(
          "http://localhost:5000/auth/commentsEcons",
          { content: newComment, Username: username }
        );
        setCommentsEcons([response.data, ...commentsEcons]);
        setNewComment("");
      } catch (error) {
