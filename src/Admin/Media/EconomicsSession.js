...
import { jss1econssessionPost } from "../../redux/Actions";
  ...
  useEffect(() => {
    const fetchjss1econssessionPosts = async () => {
      try {
        ...
        setjss1econssessionPosts(response.data);
      } catch (error) {
...
const [totalPages, setTotalPages] = useState(40);
  ...
  const handleDeletePost = async (postId) => {
    try {
      await axios.delete(`http://localhost:5000/auth/jss1econssession/${postId}`);
      setjss1econssessionPosts(jss1econssessionPosts.filter(post => post._id !== postId));
  ...
  const handleVideoChange = (index, file) => {
    setQuestions((prevQuestions) => {
      const updatedQuestions = [...prevQuestions];
      updatedQuestions[index].video = file;
  ...
  // const handleVideoChange = (index, file) => {
  //   // Assume 'file' is Blob or File object received from file input
  //   // Upload file to server and get the URL
  //   jss1econssessionPost(file).then((url) => {
  //     setQuestions((prevQuestions) => {
  //       const updatedQuestions = [...prevQuestions];
  //       updatedQuestions[index].video = url; // Set the received URL from server
  //       return updatedQuestions;
  //     });
  //   }).catch((error) => {
  ...
  const handleVideoChange2 = (index, file) => {
    setQuestions((prevQuestions) => {
      const updatedQuestions = [...prevQuestions];
      updatedQuestions[index].video2 = file;
  ...
  const handlePostQuestions = async () => {
    try {
      ...
      const response = dispatch(jss1econssessionPost(formData));
...
useEffect(() => {
  ...
  return (
    <div>
      ...
      <hr />
      {jss1econssessionPosts && jss1econssessionPosts.length > 0 ? (
        
        <div className="Details-Section"> 
      {/* {jss1econssessionPosts.map((post, index) => (  */}
         {jss1econssessionPosts.slice((currentPage - 1) * recordsPerPage, currentPage * recordsPerPage).map((post, index) => (
...
export default EconomicsSession;
